...
> 基于多 Agent 协作框架的狼人杀对战系统，支持纯 AI 对战与人机混战。

## ⚠️ 安全提示

**API Key 仅通过 `backend/.env` 环境变量配置，绝不硬编码到源代码中。**
`.env` 文件已在 `.gitignore` 中排除，不会被提交到版本控制系统。
...
本项目实现一个基于多 Agent 协作框架的狼人杀对战系统，支持纯 AI 对战与人机混战。

**核心特性：**

- 每个玩家（Agent）根据角色拥有独立目标、信息视野与行动空间
- 严格信息隔离下通过调用远程大模型进行推理、发言与决策
- 完整对局引擎、回合流转、胜负裁决及结构化日志
- 前端提供观战 UI，实时展示多智能体博弈过程（玩家状态卡片 + 事件流）
- 观战者支持中途加入并自动回放历史事件
...
1. **用户/客户端** 通过 REST 创建房间或加入房间（人机混战指定人类玩家座位）
2. **房间管理服务** 初始化 GameEngine，分配角色（狼人、预言家、女巫、村民等），为每个座位创建 Agent 实例（人类座位则创建 HumanProxyAgent）
3. **WebSocket 通道** 将前端与该房间绑定，随后所有阶段转换、发言、投票、行动结果均以 JSON 事件推送
4. **GameEngine** 按狼人杀标准规则驱动回合：
   - **夜晚阶段**：狼人刀人 → 预言家查验 → 女巫用药/毒 → 猎人/白痴等技能触发
   - **昼间阶段**：公布死亡信息 → 顺序发言 → 投票放逐 → 遗言/技能结算
5. **胜负判定**：狼人存活数 vs 神职/村民存活数
6. 每个 AI Agent 在需要决策时，由 Agent Dispatcher 调用 LLM Client，向远程大模型发送包含角色提示、历史对话、当前信息（仅限该角色可见）的 prompt，解析返回的结构化动作（JSON）
7. 人类玩家通过 PlayerActionBar 提交行动，经由 WebSocket 返回后端，引擎统一处理
8. 所有对局事件、玩家发言、决策原因等均由 **结构化日志** 记录，并可通过 API 查询或回放
...
# 编辑 .env 配置 LLM API（不要写死 API Key 到代码中）
...
```env
LLM_PROVIDER=openai          # openai（真实API）或 mock（测试模式）
LLM_API_KEY=<你的 API Key>   # ⚠️ 仅通过 .env 配置，不要硬编码到代码中
...
`HumanProxyAgent` 继承 `BaseAgent`，但不调用 LLM。其 `decide()` 方法将决策请求通过 WebSocket 发送给前端，前端弹出对应操作界面（如夜晚狼人杀人选择器、白天发言输入框），用户提交后结果返回后端继续流程。
...
可在 `agents` 模块中加入经验记忆与反思机制，将历史对局日志作为 Few-shot 示例或微调数据，实现：
...
Agent 基类设计为可插拔，支持从通用聊天 Agent 渐进加载角色设定与规则知识，实现：
