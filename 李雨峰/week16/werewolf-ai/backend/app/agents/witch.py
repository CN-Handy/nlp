...
class WitchAgent(BaseAgent):
    """AI agent playing the witch role."""

    def __init__(self, player_id: str, name: str):
        super().__init__(player_id, name, PlayerRole.WITCH)
    ...
    async def decide(
        ...
        if phase != GamePhase.WITCH_TURN:
            return AgentAction(action_type="skip", reasoning="Not witch turn")
    ...
    def _build_game_info(self, game_state: GameState) -> dict[str, Any]:
        ...
        has_heal = player.has_heal_potion if player else False
        has_poison = player.has_poison_potion if player else False
        ...
        return {
            ...
            "has_heal_potion": has_heal,
            "has_poison_potion": has_poison,
            "valid_targets": [p.player_id for p in alive_players] if has_heal or has_poison else [],
        }
