...
async def get_leaderboard_api(
    ...
    """获取排行榜"""
    lb = get_leaderboard()
    rankings = lb.get_rankings(role_type=role_type, sort_by=sort_by, limit=limit)
    role_comparison = lb.get_role_comparison()
    return {
        "rankings": rankings,
        "role_comparison": role_comparison,
        "total_entries": sum(
            len(entries) for entries in lb.entries.values()
        ),
    }
