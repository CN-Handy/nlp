...
class GameEngine:
    ...
    async def _handle_hunter_death(self, killed_player_ids: List[int]):
        """处理猎人死亡开枪（在is_alive已设为False后调用），使用AI代理决策目标"""
            ...
            if player and player.role_type == RoleType.HUNTER and player.role.can_shoot:
                ...
                if alive_players:
                    ...
                    if player.agent:
                        ...
                        decision = await player.agent.decide_night_action(game_state)
                        target = decision.get("target")
                        reasoning = decision.get("reasoning", "")

                    # 如果AI没有返回有效目标，默认选第一个存活玩家
                    if target is None or target == player_id or not self.game_state.get_player(target) or not self.game_state.get_player(target).is_alive:
                        valid_targets = [p.player_id for p in alive_players if p.player_id != player_id]
                        if valid_targets:
                            target = valid_targets[0]
                            reasoning = f"猎人{player_id}默认选择开枪目标"
    ...
    async def _vote(self):
        ...
        if vote_count:
            ...
            if len(eliminated) == 1:
                ...
                # 检查是否为白痴：白痴被投票时翻牌不死亡
                if hasattr(player.role, 'on_death'):
                    death_result = player.role.on_death("vote", self.game_state.to_dict())
                    if death_result == []:  # 白痴翻牌，不死亡
                        print(f"\n玩家 {eliminated[0]} ({player.name}) 翻开身份牌，是白痴！存活但失去投票权。")
                        self._log("log_event", f"玩家{eliminated[0]}({player.name}) 翻开身份牌-白痴，存活")
                        self.game_state.dialogues.append({
                            "day": self.game_state.day_number,
                            "phase": "投票",
                            ...
                            "role": player.role.name,
                            "action": "idiot_reveal",
                            "content": f"{player.name}翻开身份牌，是白痴！",
                        })
                        # 白痴翻牌后不死，不记录死亡
                    else:
                ...
                # PK环节：平票玩家再次发言
                await self._pk_speeches(eliminated)
                # PK后再投票
                pk_result = await self._pk_vote(eliminated)
                if pk_result is not None:
                    player = self.game_state.get_player(pk_result)
                    if player:
                        print(f"\nPK结果：玩家 {pk_result} ({player.name}) 被投票出局")
                        player.kill("vote", self.game_state.to_dict())
                        self._log("log_death", f"PK后玩家{pk_result}({player.name}) 被投票出局")
                        self.death_records.append({
                            "player_id": pk_result,
                            "player_name": player.name,
                            "role": player.role.name,
                        ...
                        await self._handle_hunter_death([pk_result])
                else:
                    print("PK后仍然平票，无人出局")
                    self._log("log_event", "PK后仍然平票，无人出局")

        self.game_state.reset_vote_record()

    async def _pk_speeches(self, eliminated: List[int]):
        """PK环节：平票玩家再次发言"""
        print(f"\n--- PK发言 ---")
        self._log("log_event", "PK发言开始")
        for pid in eliminated:
            ...
            if player and player.agent:
                ...
                # 告知PK状态
                game_state["pk_players"] = eliminated
                game_state["phase"] = "PK发言"
                decision = await player.agent.decide_speech(game_state)
                content = decision.get("content", "")
                print(f"{player.name}(PK): {content}")
                self._log("log_speech", f"{player.name}(PK): {content[:100]}...")
                self.game_state.dialogues.append({
                    "day": self.game_state.day_number,
                    "phase": "PK发言",
                    "player_id": pid,
    ...
    async def _pk_vote(self, eliminated: List[int]) -> Optional[int]:
        """PK环节：投票（只能投给PK台上的玩家）"""
        alive_players = self.game_state.get_alive_players()
        votes = {}
        print(f"\n--- PK投票 ---")
        self._log("log_event", "PK投票开始")

        for player in alive_players:
            if player.agent:
                game_state = self.game_state.get_player_private_context(player.player_id)
                game_state["pk_players"] = eliminated
                game_state["phase"] = "PK投票"
                decision = await player.agent.decide_vote(game_state)
                target = decision.get("target")
                # 只能投PK台上的玩家
                if target is not None and target in eliminated:
                    votes[player.player_id] = target
                    self._log("log_vote", f"玩家{player.player_id}(PK) 投票给 玩家{target}")
                    self.game_state.dialogues.append({
                        "day": self.game_state.day_number,
                        "phase": "PK投票",
                        "player_id": player.player_id,
        ...
        max_votes = max(vote_count.values())
        top = [p for p, c in vote_count.items() if c == max_votes]
    ...
    async def _run_summaries(self):
        ...
        # 计算评测指标并更新排行榜
        await self._compute_evaluation()
    ...
    async def _compute_evaluation(self):
        """计算评测指标并更新排行榜"""
        try:
            winner = self.game_state.get_winner()
            players = [p.to_dict() for p in self.game_state.players]
            dialogues = list(self.game_state.dialogues)
            ...
            metrics = compute_metrics(
                ...
                total_rounds=self.game_state.day_number,
            )

            # 保存评测结果
            import os
            eval_dir = os.path.join("logs", "evaluations")
            os.makedirs(eval_dir, exist_ok=True)
            import json
            eval_path = os.path.join(eval_dir, f"{self.game_id}_eval.json")
            ...
            print(f"评测指标已计算并保存至: {eval_path}")
            self._log("info", "评测指标计算完成，排行榜已更新")
        except Exception as e:
            print(f"评测计算失败: {e}")
            self._log("error", f"评测计算失败: {e}")
